...
public class JobDispatcher implements Runnable{
	...
	public void stopDispatcher() {
		...
		if (futureTaskTTLList != null)
			futureTaskTTLList.clear();
	...
	public void execute(Job job) {
		...
		{
			job.setTimeOut(System.currentTimeMillis() + jobTimeOut*1000);
		}
	...
	public JobFutureTask innExecute(Job job)
	{
		JobFutureTask ftask = null;
		
		if (job.getTimeOut() > 0)
		{
			ftask = new JobFutureTask(job, null);
	...
	public void beforeExecuteJob(Job job)
	{
		...
		//用于超时检查
		if (job instanceof JobFutureTask)
		{
			futureTaskTTLList.add((JobFutureTask)job);
			notifyHasTimeOutJob();
		}
	...
	public void releaseJob(Job job) {
		...
		if (job instanceof JobFutureTask)
		{
			futureTaskTTLList.remove((JobFutureTask)job);
		}
		...
		while(runFlag)
		{
			try
			{
				...
				else
				{
					...
					if (task != null)
						blockNoTimeOutJob(task.getTimeOut() - System.currentTimeMillis());
				}
			}
			catch(Exception ex)
			{
